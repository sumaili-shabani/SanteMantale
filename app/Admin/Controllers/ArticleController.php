...
class ArticleController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->column('title', __('Title'))->display(function($val){
        ...
        $grid->column('img', __('Image'))->image('','80', '80')->display(function($val){
            if(empty($val)){
                return "No Thumbnal";
            }
            return $val;
        ...
        $grid->filter(function($filter){
           ...
           $filter->like('ArticleType.title', __('Catégorie'));
        });
    ...
    protected function form()
    {
        ...
        $form->image('img', __('Thumbnail'))->move('/article/')->uniqueName();
        $form->UEditor('article_content','Article_content');
        $states = [
            'on'  => ['value' => 1, 'text' => 'Actif okey', 'color' => 'success'],
            'off' => ['value' => 0, 'text' => 'Actif non', 'color' => 'danger'],
        ];
